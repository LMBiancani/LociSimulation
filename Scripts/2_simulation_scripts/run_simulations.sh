...
cmd_list="$out2_1/set_$random_seed/simphy_command_list.txt"
echo "Path to SimPhy commands: $cmd_list"

# Output subdirectory for the simulations:
out_dir="$out2_2/set_$random_seed"
mkdir -p "$out_dir"
...
parallel --jobs $SLURM_CPUS_PER_TASK --joblog "$job_log" --no-notice < "$cmd_list"
...
sleep 10 # Give the filesystem a moment to catch up
...
if [ "$actual_count" -eq "$expected_count" ]; then
    echo "-------------------------------------------------------"
    echo " SUCCESS: run_simulations.sh finished successfully."
